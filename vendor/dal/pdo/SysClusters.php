 ...
 * @author Okan CIRAN
 */
class SysClusters extends \DAL\DalSlim {

    /**    
     * @author Okan CIRAN
     * @ sys_clusters tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**    
     * @author Okan CIRAN
     * @ sys_clusters tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  20.06.2016    
     * @return array
     * @throws \PDOException
     */
    public function getAll($params = array()) {
        try {
	        ...
	        LEFT JOIN sys_clusters ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND ax.language_id = lx.id 
    ...
    /**      
     * @author Okan CIRAN
     * @ sys_clusters tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  20.06.2016
     * @return array
     * @throws \PDOException
     */
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($params)) {
                    if (!isset($kontrol[0]['control'])) {                       
                        ...
                        $insertID = $pdo->lastInsertId('sys_clusters_id_seq');
                        $errorInfo = $statement->errorInfo();
                        if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                            throw new \PDOException($errorInfo[0]);
                        $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_clusters tablosunda name sutununda daha önce oluşturulmuş mu? 
    ...
    /** 
     * @author Okan CIRAN
     * sys_clusters tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  20.06.2016
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**  
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_clusters tablosundan kayıtları döndürür !!
     * @version v 1.0  20.06.2016
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /**     
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_clusters tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  20.06.2016
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /** 
     * @author Okan CIRAN
     * @ sys_clusters tablosuna aktif olan diller için ,tek bir kaydın tabloda olmayan diğer dillerdeki kayıtlarını oluşturur   !!
     * @version v 1.0  20.06.2016
     * @return array
     * @throws \PDOException
     */
    ...
    /** 
     * @author Okan CIRAN
     * @ text alanları doldurmak için sys_clusters tablosundan tek kayıt döndürür !! 
    ...
    /**  
     * @author Okan CIRAN
     * @  tree doldurmak için Sektör isimlerini sys_clusters tablosundan döndürür !!
     * @version v 1.0 20.06.2016
     * @param array | null $args
     * @return array
     * @throws \PDOException     
        ...
        try {
            ...
            $sql = "   
                SELECT                    
                    ...
                    CASE
                        (CASE 
                            (SELECT DISTINCT 1 state_type FROM sys_clusters WHERE parent_id = a.id AND deleted = 0)    
                             WHEN 1 THEN 'closed'
                             ELSE 'open'   
                             END ) 
                         WHEN 'open' THEN COALESCE(NULLIF((SELECT DISTINCT 'closed' FROM sys_clusters mz WHERE mz.parent_id = a.id AND mz.deleted = 0), ''), 'open')   
                    ELSE 'closed'
                    END AS state_type,
                    CASE
                        (SELECT DISTINCT 1 parent_id FROM sys_clusters WHERE id = a.id AND deleted = 0 AND parent_id =0 )    
                        WHEN 1 THEN 'true'
                    ELSE 'false'   
                    END AS root_type,
                    CASE 
                ...
                LEFT JOIN sys_clusters ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.language_id = lx.id
                WHERE                    
